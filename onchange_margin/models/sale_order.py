...
class SaleOrder(models.Model):
    ...
    def write(self, values): 
            for rec in self.order_line:
                ...
                # rec.unit_net_price  = round(rec.total_price/rec.product_uom_qty,2)
                if rec.product_uom_qty != 0:
                    rec.unit_net_price = round(rec.total_price / rec.product_uom_qty, 2)
                else:
                    rec.unit_net_price = 0

                rec.partner_unit_net_price  = round(rec.unit_net_price-(rec.unit_net_price*(rec.partner_discount/100)),2)
                rec.price_unit = (rec.unit_net_price * (1+ rec.conditions/100))/(1- rec.mergin/100)
