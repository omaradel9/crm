...
class QuotationReportXlsx(models.AbstractModel):
    _name = 'report.sale_order_reports.custom_sale_order_action_report_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, orders):
        ...
        for obj in orders:
            ...
            for product in obj.order_line:
                ...
                sheet.write_blank(table_row, 1, None, None)
            ...
            table_row +=1
            if obj.term_conditions_id.offer_validity:
                sheet.set_row(table_row,30)
                sheet.merge_range(table_row, 0, table_row, 2, 'Offer Validity:', format_4)
                sheet.merge_range(table_row, 3, table_row, 8, obj.term_conditions_id.offer_validity, format_4)
                table_row += 2
            if  obj.term_conditions_id.prices: 
                sheet.set_row(table_row,30) 
                sheet.merge_range(table_row, 0, table_row, 2, 'Prices:', format_4)
                sheet.merge_range(table_row, 3, table_row, 8, obj.term_conditions_id.prices, format_4)

                table_row += 2
            if obj.term_conditions_id.payment:
                sheet.set_row(table_row,30)
                soup = BeautifulSoup(obj.term_conditions_id.payment, 'html.parser')
                ...
                table_row += 2
            if obj.term_conditions_id.delivery:
                sheet.set_row(table_row,30) 
                sheet.merge_range(table_row, 0, table_row, 2, 'Delivery:', format_4)
                sheet.merge_range(table_row, 3, table_row, 8, obj.term_conditions_id.delivery, format_4)
                table_row += 2
            if obj.term_conditions_id.partial_delivery:
                sheet.set_row(table_row,30)  
                sheet.merge_range(table_row, 0, table_row, 2, 'Partial Delivery:', format_4)
                sheet.merge_range(table_row, 3, table_row, 8, obj.term_conditions_id.partial_delivery, format_4)
                table_row += 2
            if obj.term_conditions_id.responsibility:
                sheet.set_row(table_row,30)   
                sheet.merge_range(table_row, 0, table_row, 2, 'Responsibility:', format_4)
                sheet.merge_range(table_row, 3, table_row, 8, obj.term_conditions_id.responsibility, format_4)
                table_row += 2
            if obj.term_conditions_id.required_information:
                sheet.set_row(table_row,30)
                sheet.merge_range(table_row, 0, table_row, 2, 'Required Information:', format_4)
                sheet.merge_range(table_row, 3, table_row, 8, obj.term_conditions_id.required_information, format_4)
